...
先将链接转换成最终配置文件地址。最终配置文件地址为原地址+原地址最后一个"/"之后的内容+".list"，例如链接`https://github.com/blackmatrix7/ios_rule_script/tree/master/rule/Clash/ZhihuAds`的最终配置文件地址为`https://github.com/blackmatrix7/ios_rule_script/tree/master/rule/Clash/ZhihuAds/ZhihuAds.list`。得到最终配置地址之后，读取配置内容。
- 将配置信息转换为RouterOS脚本。
...
```
/ip dns static add name=voice.telephony.goog type=FWD forward-to=$vpn_dns_server address-list=vpn_list match-subdomain=no comment="vpn-dns: Google"
```
...
```
/ip dns static add name=1e100.net type=FWD forward-to=$vpn_dns_server address-list=vpn_list match-subdomain=yes comment="vpn-dns: Google"
```
...
```
/ip firewall address-list add address=74.125.0.0/16 comment="vpn: Google" list=vpn_list
/ip firewall address-list add address=2620:120:e000::/40 comment="vpn: Google" list=vpn_list
```
...
2. 在链接列表中增加导出按钮。点击导出按钮，调用后端接口将当前选中的链接转换为RouterOS脚本。并保存到文件中。导出按钮为FloatButton。